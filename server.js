...
const guardDataFile = path.join(storageDir, 'guard-data.json');
...
function seedDemoMarketplaceData(data) {
  ...
  const demoPartner = {
    ...
    ownerEmail: 'demo@teyo.ca',
    websiteUrl: 'https://northstar.example.com',
    storeCatalogUrl: 'https://northstar.example.com/products',
    storeNiche: 'Outdoor apparel',
    details: 'Demo company profile for live stock monitoring and size availability.',
    paid: true,
    activeListing: true,
    paymentConfirmed: true,
    requestStatus: 'approved',
    claimStatus: 'claimed',
    companyAccessKey: 'teyo-demo-access-key-001',
    storeSync: {
      enabled: false,
      format: 'json',
      sourceUrl: '',
  ...
  data.partners = [demoPartner];
  data.products = [
    {
      id: 'demo-product-1',
      productName: 'Apex Runner Jacket',
      companyName: demoPartner.companyName,
      ownerEmail: demoPartner.ownerEmail,
      category: 'outerwear',
      price: '$189',
      websiteUrl: demoPartner.websiteUrl,
      description: 'Lightweight technical jacket with weatherproof shell and room for layering.',
      imageUrl: '',
      stockStatus: 'In stock',
      safetyNote: 'Online and in-store availability is updated daily.',
      stores: ['Toronto Flagship', 'Vancouver Studio'],
      sizeOptions: ['S', 'M', 'L', 'XL'],
      sizeInventory: [
        { storeName: 'Toronto Flagship', size: 'M', stockStatus: 'In stock', restockDate: '' },
        { storeName: 'Toronto Flagship', size: 'L', stockStatus: 'Limited', restockDate: '2026-08-12' },
        { storeName: 'Vancouver Studio', size: 'M', stockStatus: 'In stock', restockDate: '' },
        { storeName: 'Vancouver Studio', size: 'XL', stockStatus: 'Back soon', restockDate: '2026-08-08' }
      ],
      rating: '4.8',
      reviewCount: '121',
      verifiedSeller: true,
      verificationStatus: 'Verified seller',
      trustSummary: 'Live size availability and store monitoring are enabled.',
    ...
    {
      id: 'demo-product-2',
      productName: 'Summit Shell Pant',
      companyName: demoPartner.companyName,
      ownerEmail: demoPartner.ownerEmail,
      category: 'pants',
      price: '$129',
      websiteUrl: demoPartner.websiteUrl,
      description: 'Water-resistant shell pant with fitted knee articulation for climbs and travel.',
      imageUrl: '',
      stockStatus: 'Limited',
      safetyNote: 'Sizing updates are visible for customers and companies in the same dashboard.',
      stores: ['Toronto Flagship', 'Montreal Pop-Up'],
      sizeOptions: ['S', 'M', 'L'],
      sizeInventory: [
        { storeName: 'Toronto Flagship', size: 'S', stockStatus: 'Limited', restockDate: '2026-08-10' },
        { storeName: 'Toronto Flagship', size: 'L', stockStatus: 'In stock', restockDate: '' },
        { storeName: 'Montreal Pop-Up', size: 'M', stockStatus: 'Back soon', restockDate: '2026-08-07' }
      ],
      rating: '4.6',
      reviewCount: '88',
      verifiedSeller: true,
      verificationStatus: 'Verified seller',
      trustSummary: 'Customers can browse size-specific restock windows before purchasing.',
      hasPhysicalStore: true,
      physicalStoreLocation: 'Montreal, QC',
      approved: true,
      visible: true,
      createdAt: new Date().toISOString()
    },
    {
      ...
      safetyNote: 'Customers can see when online and nearby stores will restock this item.',
      stores: ['Toronto Flagship', 'Online'],
      sizeOptions: ['OS'],
      sizeInventory: [
        { storeName: 'Toronto Flagship', size: 'OS', stockStatus: 'Back soon', restockDate: '2026-08-09' },
...
function ensureDataShape(parsed) {
  ...
  seedDemoMarketplaceData(result);
  return result;
}
...
function readLocalDataFallback() {
  ...
  try {
    ...
    const shouldPersist = (!Array.isArray(parsed?.partners) || parsed.partners.length === 0)
    ...
    seedDemoMarketplaceData(fallbackData);
    fs.writeFileSync(dataFile, JSON.stringify(fallbackData, null, 2));
    return fallbackData;
  }
}

function getDefaultGuardData() {
  return {
    entities: [],
    alerts: []
  };
...
function ensureGuardDataShape(parsed) {
  const entities = Array.isArray(parsed?.entities) ? parsed.entities : [];
  return {
    entities: entities.map((entity) => ({
      id: sanitizePlainText(entity?.id, 120),
      email: sanitizeEmail(entity?.email) || '',
      companyName: sanitizePlainText(entity?.companyName, 180),
      aum: Number.isFinite(Number(entity?.aum)) ? Number(entity?.aum) : 0,
      primaryResidency: sanitizePlainText(entity?.primaryResidency, 40) || 'CA',
      riskTolerance: sanitizePlainText(entity?.riskTolerance, 80) || 'Conservative_Preservation',
      accessKey: sanitizePlainText(entity?.accessKey, 160),
      plan: sanitizePlainText(entity?.plan, 40) || 'guard',
      subscriptionStatus: sanitizePlainText(entity?.subscriptionStatus, 40) || 'inactive',
      stripeCustomerId: sanitizePlainText(entity?.stripeCustomerId, 120),
      stripeSessionId: sanitizePlainText(entity?.stripeSessionId, 120),
      lastCheckoutAt: sanitizePlainText(entity?.lastCheckoutAt, 80),
      createdAt: sanitizePlainText(entity?.createdAt, 80) || new Date().toISOString(),
      updatedAt: sanitizePlainText(entity?.updatedAt, 80) || new Date().toISOString(),
      locations: Array.isArray(entity?.locations) ? entity.locations.map((entry) => ({
        id: sanitizePlainText(entry?.id, 120),
        jurisdiction: sanitizePlainText(entry?.jurisdiction, 80),
        daysThisYear: Number.isFinite(Number(entry?.daysThisYear)) ? Number(entry?.daysThisYear) : 0,
        daysLastYear: Number.isFinite(Number(entry?.daysLastYear)) ? Number(entry?.daysLastYear) : 0,
        daysTwoYearsAgo: Number.isFinite(Number(entry?.daysTwoYearsAgo)) ? Number(entry?.daysTwoYearsAgo) : 0,
        createdAt: sanitizePlainText(entry?.createdAt, 80) || new Date().toISOString()
      })) : [],
      holdings: Array.isArray(entity?.holdings) ? entity.holdings.map((entry) => ({
        id: sanitizePlainText(entry?.id, 120),
        type: sanitizePlainText(entry?.type, 80),
        jurisdiction: sanitizePlainText(entry?.jurisdiction, 80),
        valuationUSD: Number.isFinite(Number(entry?.valuationUSD)) ? Number(entry?.valuationUSD) : 0,
        ownership: sanitizePlainText(entry?.ownership, 80),
        custodian: sanitizePlainText(entry?.custodian, 80),
        createdAt: sanitizePlainText(entry?.createdAt, 80) || new Date().toISOString()
      })) : [],
      alerts: Array.isArray(entity?.alerts) ? entity.alerts.map((entry) => ({
        id: sanitizePlainText(entry?.id, 120),
        severity: sanitizePlainText(entry?.severity, 40) || 'warning',
        message: sanitizePlainText(entry?.message, 500),
        createdAt: sanitizePlainText(entry?.createdAt, 80) || new Date().toISOString()
      })) : []
    })).filter((entity) => entity.id),
    alerts: Array.isArray(parsed?.alerts) ? parsed.alerts.map((entry) => ({
      id: sanitizePlainText(entry?.id, 120),
      entityId: sanitizePlainText(entry?.entityId, 120),
      severity: sanitizePlainText(entry?.severity, 40) || 'warning',
      message: sanitizePlainText(entry?.message, 500),
      createdAt: sanitizePlainText(entry?.createdAt, 80) || new Date().toISOString()
    })) : []
...
function ensureGuardStorageFile() {
  if (!fs.existsSync(storageDir)) {
    fs.mkdirSync(storageDir, { recursive: true });
  }

  if (!fs.existsSync(guardDataFile)) {
    fs.writeFileSync(guardDataFile, JSON.stringify(getDefaultGuardData(), null, 2));
  }
}

function readLocalGuardData() {
  ensureGuardStorageFile();

  try {
    const parsed = JSON.parse(fs.readFileSync(guardDataFile, 'utf8'));
    return ensureGuardDataShape(parsed);
  } catch (error) {
    return getDefaultGuardData();
  }
}

function getGuardEncryptionKey() {
  return crypto.createHash('sha256').update(String(process.env.GUARD_ENCRYPTION_KEY || 'teyo-guard-v1-default-key')).digest();
}
...
function encryptSensitiveValue(value) {
  ...
  const cipher = crypto.createCipheriv('aes-256-gcm', getGuardEncryptionKey(), iv);
...
function decryptSensitiveValue(value) {
  ...
  const [ivHex, encryptedHex, tagHex] = input.split(':');
  ...
  try {
    ...
    const decipher = crypto.createDecipheriv('aes-256-gcm', getGuardEncryptionKey(), iv);
...
function generateEntityId(email, companyName) {
  const seed = `${sanitizeEmail(email) || 'unknown'}:${sanitizePlainText(companyName, 180) || 'entity'}:${Date.now()}`;
  return `ent_${crypto.createHash('sha256').update(seed).digest('hex').slice(0, 12)}`;
}

function calculateUsSubstantialPresence(daysCurrentYear, daysPriorYear, daysTwoYearsPrior) {
  const weightedDays = Number(daysCurrentYear || 0) + (Number(daysPriorYear || 0) / 3) + (Number(daysTwoYearsPrior || 0) / 6);
  const taxLiabilityTriggered = weightedDays >= 183;
  const daysUntilThreshold = Math.max(0, 183 - weightedDays);

  return {
    weightedTotal: Number(weightedDays.toFixed(2)),
    taxLiabilityTriggered,
    daysUntilThreshold: Number(daysUntilThreshold.toFixed(1))
  };
}

function buildPortfolioStress(holdings) {
  const safeHoldings = Array.isArray(holdings) ? holdings : [];
  const totalPortfolioUSD = safeHoldings.reduce((sum, entry) => sum + (Number(entry?.valuationUSD) || 0), 0);
  const liquidUSD = safeHoldings.reduce((sum, entry) => {
    const type = String(entry?.type || '').toLowerCase();
    const isLiquid = type.includes('equity') || type.includes('cash') || type.includes('crypto');
    return sum + (isLiquid ? Number(entry?.valuationUSD) || 0 : 0);
  }, 0);
  const illiquidUSD = Math.max(0, totalPortfolioUSD - liquidUSD);
  const liquidRatio = totalPortfolioUSD > 0 ? Math.round((liquidUSD / totalPortfolioUSD) * 100) : 0;
  ...
  return {
    ...
    liquidRatio,
    stressScenarios: {
      mildDownturn: { label: 'Mild 8% downturn', totalExposure: Math.round(totalPortfolioUSD * 0.08) },
      severeCorrection: { label: 'Severe 15% correction', totalExposure: Math.round(totalPortfolioUSD * 0.15) },
      crisis: { label: 'Crisis 24% drawdown', totalExposure: Math.round(totalPortfolioUSD * 0.24) }
    }
  };
}

function applyGuardSubscriptionState(entity, options = {}) {
  if (!entity) {
    return null;
  }

  entity.plan = sanitizePlainText(options.plan, 40) || 'guard';
  entity.subscriptionStatus = sanitizePlainText(options.subscriptionStatus, 40) || 'active';
  entity.stripeCustomerId = sanitizePlainText(options.stripeCustomerId || entity.stripeCustomerId || '', 120);
  entity.stripeSessionId = sanitizePlainText(options.stripeSessionId || entity.stripeSessionId || '', 120);
  entity.lastCheckoutAt = sanitizePlainText(options.lastCheckoutAt || entity.lastCheckoutAt || '', 80) || new Date().toISOString();
  entity.updatedAt = new Date().toISOString();
  return entity;
}

async function activateGuardEntitySubscription(entityId, options = {}) {
  ...
  applyGuardSubscriptionState(entity, {
    plan: 'guard',
    subscriptionStatus: 'active',
    stripeCustomerId: options.stripeCustomerId || entity.stripeCustomerId || '',
    stripeSessionId: options.stripeSessionId || entity.stripeSessionId || '',
    lastCheckoutAt: options.lastCheckoutAt || entity.lastCheckoutAt || new Date().toISOString()
  });

  const successMessage = 'TEYO Guard subscription activated. Compliance monitoring is now live.';
  const hasMessage = entity.alerts.some((entry) => entry.message === successMessage);
  if (!hasMessage) {
    entity.alerts.push({
      id: `alert-${Date.now()}`,
      severity: 'info',
  ...
  state.alerts = state.entities.flatMap((entry) => entry.alerts.map((alert) => ({ ...alert, entityId: entry.id })));
  guardState = state;
  await persistGuardState();
  return entity;
}

function buildGuardDashboard(entity) {
  const locations = Array.isArray(entity?.locations) ? entity.locations : [];
  const holdings = Array.isArray(entity?.holdings) ? entity.holdings : [];
  const spt = calculateUsSubstantialPresence(
    locations.reduce((sum, entry) => sum + (Number(entry?.daysThisYear) || 0), 0),
    locations.reduce((sum, entry) => sum + (Number(entry?.daysLastYear) || 0), 0),
    locations.reduce((sum, entry) => sum + (Number(entry?.daysTwoYearsAgo) || 0), 0)
  );
  const warnings = [];
  if (spt.taxLiabilityTriggered) {
    warnings.push('🚨 US substantial presence threshold triggered. Prepare cross-border tax documentation immediately.');
  } else if (spt.daysUntilThreshold <= 14) {
    warnings.push('⚠️ US substantial presence threshold is within 14 days. Review residency exposure now.');
  } else {
    warnings.push('✅ Residency exposure remains within normal monitoring thresholds.');
  }

  if (Number(entity?.aum || 0) >= 50000000) {
    warnings.push('⚠️ Large AUM profile requires enhanced liquidity and trust governance review.');
  }

  if (!holdings.length) {
    warnings.push('⚠️ Add holdings to model portfolio stress and liquidity exposure.');
  }
  ...
  return {
    ...
    email: entity?.email || '',
    aum: Number(entity?.aum || 0),
    plan: entity?.plan || 'guard',
    subscriptionStatus: entity?.subscriptionStatus || 'inactive',
    stripeCustomerId: entity?.stripeCustomerId || '',
    stripeSessionId: entity?.stripeSessionId || '',
    lastCheckoutAt: entity?.lastCheckoutAt || '',
    taxStatus: spt.taxLiabilityTriggered ? 'Cross-border exposure detected' : 'Active monitoring',
    sptScore: spt.weightedTotal,
    daysRemaining: spt.daysUntilThreshold,
    warnings,
    portfolioStress: buildPortfolioStress(holdings)
  };
}

async function initializeGuardStorage() {
  if (!databaseUrl || !dbPool) {
    guardState = readLocalGuardData();
    guardStorageMode = 'file';
    return;
  }

  try {
    await dbPool.query(`
      CREATE TABLE IF NOT EXISTS teyo_guard_entities (
        id TEXT PRIMARY KEY,
        email TEXT NOT NULL,
        company_name TEXT NOT NULL,
        aum NUMERIC NOT NULL DEFAULT 0,
        primary_residency TEXT NOT NULL,
    ...
    `);
    await dbPool.query(`
      CREATE TABLE IF NOT EXISTS teyo_guard_locations (
        id BIGSERIAL PRIMARY KEY,
        entity_id TEXT NOT NULL REFERENCES teyo_guard_entities(id) ON DELETE CASCADE,
        jurisdiction TEXT NOT NULL,
        days_this_year INT NOT NULL DEFAULT 0,
        days_last_year INT NOT NULL DEFAULT 0,
        days_two_years_ago INT NOT NULL DEFAULT 0,
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
      )
    `);
    await dbPool.query(`
      CREATE TABLE IF NOT EXISTS teyo_guard_holdings (
        id BIGSERIAL PRIMARY KEY,
        entity_id TEXT NOT NULL REFERENCES teyo_guard_entities(id) ON DELETE CASCADE,
        asset_type TEXT NOT NULL,
        jurisdiction TEXT NOT NULL,
        valuation_usd NUMERIC NOT NULL DEFAULT 0,
    ...
    `);
    await dbPool.query(`
      CREATE TABLE IF NOT EXISTS teyo_guard_alerts (
        id BIGSERIAL PRIMARY KEY,
        entity_id TEXT NOT NULL REFERENCES teyo_guard_entities(id) ON DELETE CASCADE,
    ...
    const entityRows = await dbPool.query(`
      SELECT id, email, company_name, aum, primary_residency, risk_tolerance, access_key_encrypted, membership_plan, subscription_status, stripe_customer_id, stripe_session_id, last_checkout_at, created_at, updated_at
      FROM teyo_guard_entities
      ORDER BY created_at ASC
    `);

    const entities = [];
    for (const row of entityRows.rows) {
      const [locationsRows, holdingsRows, alertRows] = await Promise.all([
        dbPool.query(`
          SELECT id, jurisdiction, days_this_year, days_last_year, days_two_years_ago, created_at
          FROM teyo_guard_locations
          WHERE entity_id = $1
          ORDER BY created_at ASC
        `, [row.id]),
        dbPool.query(`
          SELECT id, asset_type, jurisdiction, valuation_usd, ownership_structure, custodian, created_at
          FROM teyo_guard_holdings
          WHERE entity_id = $1
          ORDER BY created_at ASC
        `, [row.id]),
        dbPool.query(`
          SELECT id, severity, message, created_at
          FROM teyo_guard_alerts
          WHERE entity_id = $1
          ORDER BY created_at ASC
        `, [row.id])
      ]);
      ...
      entities.push({
        ...
        primaryResidency: row.primary_residency,
        riskTolerance: row.risk_tolerance,
        accessKey: decryptSensitiveValue(row.access_key_encrypted),
        plan: row.membership_plan || 'basic',
        ...
        locations: locationsRows.rows.map((entry) => ({
          id: String(entry.id),
          jurisdiction: entry.jurisdiction,
          daysThisYear: Number(entry.days_this_year || 0),
          daysLastYear: Number(entry.days_last_year || 0),
          daysTwoYearsAgo: Number(entry.days_two_years_ago || 0),
          createdAt: entry.created_at
        })),
        holdings: holdingsRows.rows.map((entry) => ({
          id: String(entry.id),
          type: entry.asset_type,
          jurisdiction: entry.jurisdiction,
          valuationUSD: Number(entry.valuation_usd || 0),
          ownership: entry.ownership_structure,
          custodian: entry.custodian,
          createdAt: entry.created_at
        })),
        alerts: alertRows.rows.map((entry) => ({
          id: String(entry.id),
    ...
    guardState = ensureGuardDataShape({ entities, alerts: [] });
    guardStorageMode = 'postgres';
  } catch (error) {
    guardState = readLocalGuardData();
    guardStorageMode = 'file';
    console.warn(`TEYO Guard storage unavailable, falling back to file storage: ${error.message}`);
  }
}

async function persistGuardState() {
  const normalizedState = ensureGuardDataShape(guardState || getDefaultGuardData());
  guardState = normalizedState;

  if (guardStorageMode === 'postgres' && dbPool) {
    try {
      await dbPool.query('DELETE FROM teyo_guard_alerts');
      await dbPool.query('DELETE FROM teyo_guard_holdings');
      await dbPool.query('DELETE FROM teyo_guard_locations');
      await dbPool.query('DELETE FROM teyo_guard_entities');

      for (const entity of normalizedState.entities) {
        await dbPool.query(`
          INSERT INTO teyo_guard_entities (id, email, company_name, aum, primary_residency, risk_tolerance, access_key_encrypted, membership_plan, subscription_status, stripe_customer_id, stripe_session_id, last_checkout_at, created_at, updated_at)
        ...
        `, [
          ...
          entity.companyName,
          Number(entity.aum || 0),
          entity.primaryResidency,
          entity.riskTolerance,
          encryptSensitiveValue(entity.accessKey),
          entity.plan || 'basic',
          entity.subscriptionStatus || 'inactive',
          entity.stripeCustomerId || '',
          entity.stripeSessionId || '',
          entity.lastCheckoutAt || null,
          entity.createdAt || new Date().toISOString(),
          entity.updatedAt || new Date().toISOString()
        ]);

        for (const location of Array.isArray(entity.locations) ? entity.locations : []) {
          await dbPool.query(`
            INSERT INTO teyo_guard_locations (entity_id, jurisdiction, days_this_year, days_last_year, days_two_years_ago, created_at)
            VALUES ($1, $2, $3, $4, $5, $6)
          `, [entity.id, location.jurisdiction, Number(location.daysThisYear || 0), Number(location.daysLastYear || 0), Number(location.daysTwoYearsAgo || 0), location.createdAt || new Date().toISOString()]);
        }

        for (const holding of Array.isArray(entity.holdings) ? entity.holdings : []) {
          await dbPool.query(`
            INSERT INTO teyo_guard_holdings (entity_id, asset_type, jurisdiction, valuation_usd, ownership_structure, custodian, created_at)
            VALUES ($1, $2, $3, $4, $5, $6, $7)
          `, [entity.id, holding.type, holding.jurisdiction, Number(holding.valuationUSD || 0), holding.ownership, holding.custodian, holding.createdAt || new Date().toISOString()]);
        }

        for (const alert of Array.isArray(entity.alerts) ? entity.alerts : []) {
          await dbPool.query(`
            INSERT INTO teyo_guard_alerts (entity_id, severity, message, created_at)
            VALUES ($1, $2, $3, $4)
          `, [entity.id, alert.severity, alert.message, alert.createdAt || new Date().toISOString()]);
        }
      }
      ...
      guardStorageMode = 'file';
      console.warn(`TEYO Guard DB persistence failed, switching to local file storage: ${error.message}`);
    }
  }

  ensureGuardStorageFile();
  fs.writeFileSync(guardDataFile, JSON.stringify(normalizedState, null, 2));
}

async function loadGuardState() {
  if (guardState) {
    return ensureGuardDataShape(guardState);
  }

  if (guardStorageMode === 'postgres' && dbPool) {
    await initializeGuardStorage();
  } else {
    guardState = readLocalGuardData();
  }

  return ensureGuardDataShape(guardState || getDefaultGuardData());
}
...
async function deliverLeadFollowUpEmail(lead) {
  const recipient = sanitizeEmail(lead?.email || '');
  if (!recipient) {
    return { delivered: false, status: 'queued', recipient: '', reason: 'No lead email on file.' };
  }

  const transporter = getReminderTransporter();
  if (!transporter) {
    return { delivered: false, status: 'queued', recipient, reason: 'SMTP delivery is not configured. Follow-up queued for later.' };
  }
  ...
  const subject = 'Welcome to TEYO — your next step is ready';
  const text = [
    `Hi ${sanitizePlainText(lead?.name || 'there')},`,
    '',
    'Thanks for showing interest in TEYO. We have captured your request and will follow up shortly.',
    'If you want to move faster, you can visit https://teyo.ca/partners to start the partner or company onboarding flow.',
    '',
    'Best,',
    'The TEYO team'
  ].join('\n');
  const html = `
    <div style="font-family: Inter, Arial, sans-serif; color: #111827; line-height: 1.6;">
      <h3 style="margin-bottom: 8px;">Welcome to TEYO</h3>
      <p>Hi ${sanitizeHtml(sanitizePlainText(lead?.name || 'there', 120))},</p>
      <p>Thanks for showing interest in TEYO. We have captured your request and will follow up shortly.</p>
      <p>If you want to move faster, you can visit <a href="https://teyo.ca/partners" target="_blank" rel="noreferrer">https://teyo.ca/partners</a> to start the partner or company onboarding flow.</p>
      <p>Best,<br />The TEYO team</p>
    </div>
  `;
  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (plan === 'placement') {
        ...
        await activateGuardEntitySubscription(metadata.entityId || '', {
...
app.post('/api/guard/entities', async (req, res) => {
  ...
  const companyName = sanitizePlainText(req.body?.companyName, 180);
  const aum = Number(req.body?.aum || 0);
  const primaryResidency = sanitizePlainText(req.body?.primaryResidency, 40) || 'CA';
  const riskTolerance = sanitizePlainText(req.body?.riskTolerance, 80) || 'Conservative_Preservation';

  if (!email || !companyName || !Number.isFinite(aum) || aum <= 0) {
    return res.status(400).json({ success: false, message: 'Please provide a valid business email, company name, and AUM.' });
  }

  const state = await loadGuardState();
  const entityId = generateEntityId(email, companyName);
  ...
  const entity = {
    ...
    locations: [],
    holdings: [],
    alerts: [{ id: `alert-${Date.now()}`, severity: 'info', message: 'Account created. Add travel days and holdings to begin compliance monitoring.', createdAt: nowIso }]
  };
  ...
  return res.json({ success: true, entityId, accessKey, message: 'TEYO Guard account created.' });
});

app.get('/api/guard/entities/:entityId', async (req, res) => {
  const entityId = sanitizePlainText(req.params?.entityId, 120);
  const accessKey = sanitizePlainText(req.headers['x-access-key'], 160);

  if (!entityId || !accessKey) {
    return res.status(401).json({ success: false, message: 'Access key is required.' });
  }
...
app.post('/api/guard/entities/:entityId/locations', async (req, res) => {
  const entityId = sanitizePlainText(req.params?.entityId, 120);
  const accessKey = sanitizePlainText(req.headers['x-access-key'], 160);
  const jurisdiction = sanitizePlainText(req.body?.jurisdiction, 80);
  const daysThisYear = Number(req.body?.daysThisYear || 0);
  const daysLastYear = Number(req.body?.daysLastYear || 0);
  const daysTwoYearsAgo = Number(req.body?.daysTwoYearsAgo || 0);

  if (!entityId || !accessKey || !jurisdiction) {
    return res.status(400).json({ success: false, message: 'Entity ID, access key, and jurisdiction are required.' });
  }
  ...
  return res.json({ success: true, message: 'Location data updated.' });
});

app.post('/api/guard/entities/:entityId/holdings', async (req, res) => {
  const entityId = sanitizePlainText(req.params?.entityId, 120);
  const accessKey = sanitizePlainText(req.headers['x-access-key'], 160);
  const type = sanitizePlainText(req.body?.type, 80);
  const jurisdiction = sanitizePlainText(req.body?.jurisdiction, 80);
  const valuationUSD = Number(req.body?.valuationUSD || 0);
  const ownership = sanitizePlainText(req.body?.ownership, 80);
  const custodian = sanitizePlainText(req.body?.custodian, 80);

  if (!entityId || !accessKey || !type || !jurisdiction || !Number.isFinite(valuationUSD) || valuationUSD <= 0) {
    return res.status(400).json({ success: false, message: 'Valid holding details are required.' });
  }
  ...
  return res.json({ success: true, message: 'Holding added.' });
});
...
app.get('/api/guard/compliance-dashboard/:entityId', async (req, res) => {
  ...
  return res.json({ success: true, dashboard: buildGuardDashboard(entity) });
});
...
app.post('/api/leads', async (req, res) => {
  ...
  const email = sanitizeEmail(req.body?.email);
  const company = sanitizePlainText(req.body?.company, 160);
  const interest = sanitizePlainText(req.body?.interest, 160);
  const referralCode = sanitizePlainText(req.body?.referralCode, 120);
  const source = sanitizePlainText(req.body?.source, 120);
  ...
  if (data.leads.length > 5000) {
    data.leads = data.leads.slice(-5000);
  }
  saveData(data);

  try {
    await deliverLeadFollowUpEmail(lead);
  } catch (emailError) {
    console.warn('Lead follow-up email failed:', emailError.message);
  }
...
app.post('/api/leads/:id/contacted', requireAdmin, (req, res) => {
  const data = loadData();
  const lead = data.leads.find((entry) => String(entry.id) === String(req.params.id));
  if (!lead) {
...
app.post('/api/partner', async (req, res) => {
  ...
  if (!companyName || !ownerEmail || !normalizedWebsite || !isBusinessOwnerSubmission(companyName, normalizedWebsite)) {
    const reason = !companyName ? 'company name missing' : (!ownerEmail ? 'owner email missing' : (!normalizedWebsite ? 'website missing' : 'website failed business validation'));
    console.warn(`Partner submission rejected: ${reason}`);
    return res.status(400).json({
      success: false,
...
app.post('/api/create-guard-checkout-session', async (req, res) => {
  if (!stripe) {
    return res.status(501).json({ success: false, message: 'Stripe is not configured yet. Add STRIPE_SECRET_KEY to enable TEYO Guard checkout.' });
  }

  try {
    const entityId = sanitizePlainText(req.body.entityId, 120);
    const accessKey = sanitizePlainText(req.body.accessKey, 160);
    const companyName = sanitizePlainText(req.body.companyName, 180);
    const ownerEmail = sanitizeEmail(req.body.ownerEmail);
    const aum = Number(req.body.aum || 0);
    const primaryResidency = sanitizePlainText(req.body.primaryResidency, 40) || 'CA';
    ...
    if (!entityId || !accessKey || !companyName || !ownerEmail) {
      return res.status(400).json({ success: false, message: 'Entity details are required before starting TEYO Guard checkout.' });
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          currency: 'usd',
          product_data: {
            name: 'TEYO Guard Annual Protection',
            description: PRICING.guard.description
          },
          unit_amount: PRICING.guard.amount,
          recurring: { interval: 'year' }
        },
        quantity: 1
      }],
      success_url: `${successBase}/guard-dashboard.html?checkout=success&plan=guard`,
      cancel_url: `${successBase}/guard-dashboard.html?checkout=cancelled&plan=guard`
    });

    res.json({ success: true, url: session.url, sessionId: session.id });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Unable to create TEYO Guard checkout session.' });
  }
});
...
app.get('/sitemap.xml', (req, res) => {
  ...
  const staticPages = [
    ...
    { loc: `${baseUrl}/guard-landing.html`, changefreq: 'monthly', priority: '0.95' },
    { loc: `${baseUrl}/guard-dashboard.html`, changefreq: 'monthly', priority: '0.85' },
...
app.get('/guard', (req, res) => {
  res.sendFile(path.join(__dirname, 'guard-landing.html'));
});

app.get('/guard-landing', (req, res) => {
  res.sendFile(path.join(__dirname, 'guard-landing.html'));
});

app.get('/guard-landing.html', (req, res) => {
  res.sendFile(path.join(__dirname, 'guard-landing.html'));
});
...
app.get('*', (req, res) => {
  ...
  res.status(404).sendFile(path.join(__dirname, 'guard-landing.html'));
});
